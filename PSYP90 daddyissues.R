...
###Try to convert to factors(numeric)
bul$EmS1 <-as.numeric(factor(bul$EmS1))
bul$EmS2 <-as.numeric(factor(bul$EmS2))
bul$EmS3 <-as.numeric(factor(bul$EMS3))
bul$EmS4 <-as.numeric(factor(bul$EMS4))
bul$EmS5 <-as.numeric(factor(bul$EmS5))



###Correlation 
variables.df <- data.frame(fdaddy_issues$RCSE, fdaddy_issues$AMORE_E, fdaddy_issues$AMORE_P, fdaddy_issues$FIS, fdaddy_issues$MIS)
variables.df
...
rcorr(fdaddy_issues$RCSE, fdaddy_issues$FIS, type= "pearson")
...
FIS.RCSE_model = lm(RCSE ~ FIS, data = fdaddy_issues)
...
residuals_RCSE = enframe(residuals(FIS.RCSE_model))
...
summary(FIS.RCSE_model)
sum(abs(fdaddy_issues$RCSE - predict(FIS.RCSE_model))^2)

FIS_df <- as.tibble(RCSE)

predictions = predict(FIS.RCSE_model, newdata = FIS_df)
...
##AMORE E model
FIS.AMORE.e_model = lm(AMORE_E ~ FIS, data = fdaddy_issues)
...
residuals_AMORE.e = enframe(residuals(FIS.AMORE.e_model))
...
describe(residuals(FIS.AMORE.e_model))
...
##AMORE P model
FIS.AMORE.p_model = lm(AMORE_P ~ FIS, data = fdaddy_issues)
...
residuals_AMORE.p = enframe(residuals(FIS.AMORE.p_model))
...
describe(residuals(FIS.AMORE.p_model))

boxplot(residuals(FIS.AMORE.p_model))

shapiro.test(residuals(FIS.AMORE.p_model))
...
##MIS;RCSE
MIS_RCSE_model <- lm(RCSE ~ MIS, data=fdaddy_issues)

as.vector(FIS)

anova(MIS_RCSE_model,RCSE_model)
...
residuals_mis.rcse = enframe(residuals(MIS_RCSE_model))
...
describe(residuals(MIS_RCSE_model))
...
MIS_RCSE_model %>%
  plot(which = 3)
ncvTest(MIS_RCSE_model)
bptest(MIS_RCSE_model)

##MIS,AMORE-E
MIS_AMORE.E_model <- lm(AMORE_E ~ MIS, data = fdaddy_issues)
MIS_AMORE.E_model
...
residuals_mis.amore.e = enframe(residuals(MIS_AMORE.E_model))

residuals_mis.amore.e %>%
  ggplot() +
  aes(x = value) +
  geom_histogram()

describe(residuals(MIS_AMORE.E_model))
...
MIS_AMORE.E_model %>%
  plot(which = 3)
ncvTest(MIS_AMORE.E_model)
bptest(MIS_AMORE.E_model)

##MIS;AMORE-p
MIS_AMORE.P_model <- lm(AMORE_P ~ MIS, data = fdaddy_issues)
MIS_AMORE.P_model
...
residuals_mis.amore.p = enframe(residuals(MIS_AMORE.P_model))

residuals_mis.amore.p %>%
  ggplot() +
  aes(x = value) +
  geom_histogram()

describe(residuals(MIS_AMORE.P_model))

#Linearity
#Kurtosis violated (-1.15)
MIS_AMORE.P_model %>%
  residualPlots()
...
MIS_AMORE.P_model %>%
  plot(which = 3)
ncvTest(MIS_AMORE.P_model)
bptest(MIS_AMORE.P_model)
...
FIS_Model <- lm(cbind(RCSE, AMORE_E, AMORE_P) ~ FIS , data = fdaddy_issues)
summary(FIS_Model)

##MIS Model 

MIS_Model <- lm(cbind(RCSE, AMORE_E, AMORE_P) ~ MIS , data = fdaddy_issues)
summary(MIS_Model)
